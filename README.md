# Hi, I'm Shraddha 👋

I'm a software engineer and doctor based in the UK. I am interested in technology and how we can build useful things to make a difference! 

🌐 **[Explore my portfolio](https://shraddha-portfolio-nine.vercel.app/)**  
💼 **[Connect with me on LinkedIn](https://www.linkedin.com/in/shraddha-ranjan-362502373/)**  

## About Me

- 🩺 Foundation doctor
- 💻 Focused on backend, full-stack, and distributed systems
- 🌍 Based in the United Kingdom
- 🚀 Open to software engineering opportunities

I developed an interest in technology from an unexpected challenge – I had to find out how to convert thousands of Word files to PDFs. After finding a macro and watching how what took ages happened in seconds, I realized how much power lies in code.

Since then, I kept learning on my own and started working on projects in Python, Java and JavaScript. I am making this change because I have found out that I am most happy when I build, create and solve problems. I enjoy the process of understanding the way things work and creating useful solutions.

## Technical Skills

**Languages**  
Python · Java · TypeScript · JavaScript · Go · OCaml · SQL · HTML/CSS

**Frontend**  
React · Next.js · Tailwind CSS · Thymeleaf · Three.js

**Backend**  
Spring Boot · Spring MVC · Spring Security · Node.js · Express.js · REST APIs

**Databases & Persistence**  
PostgreSQL · MySQL · MongoDB · Prisma · JPA · Hibernate

**Cloud, Testing & Tools**  
AWS · Docker · Git · GitHub · Linux · OpenTelemetry · Grafana · Playwright · Dune · Alcotest · QCheck

**Data, AI & Automation**  
PyTorch · Scikit-learn · Pandas · MCP · Brave API · Polygon API · Streamlit

## Current Focus

- Backend engineering
- Distributed systems
- Reliable workflow design
- Functional programming
- Performance and observability
- Building thoughtful software for real-world problems
